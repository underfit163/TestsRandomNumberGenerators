...
public class RandomExcursionsTest implements Test {
    ...
    public void runTest() {
        ...
        for (int i = 0; i < numberSample.getCountSample(); i++) {
            ...
            if (otherCount >= stateX.length * 0.8) count++;
        }
        ...
        Arrays.sort(arrPVal);
        double xi2Pvalue = ksTest.kolmogorovSmirnovTest(new UniformRealDistribution(0, 1), arrPVal);
...
//        xi2Pvalue = Gamma.regularizedGammaQ((double) (10 - 1) / 2, xi2Pvalue / 2);
        ...
        double temp = (double) count / numberSample.getCountSample();
        if (xi2Pvalue >= paramsTest.getA() || temp >= 0.95) {
        ...
        paramsTest.getDols().put(getClass().getSimpleName(), temp);
